...
class LiveFaceAuth {
  ...
  final String _serverBaseUrl = "http://localhost:8000"; // Should be configured via constructor
  String? _apiKey;
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data['is_valid'] == true) {
            await _storage.write(key: "key_validated_at", value: DateTime.now().toIso8601String());
          } else {
            throw Exception("API Key is invalid or inactive");
          }
        }
      } catch (e) {
        print("Could not validate key online. Continuing with previously validated settings if available.");
      }
    }
  }
  ...
  Future<bool> checkPassiveLiveness({required String imageBase64}) async {
    ...
    try {
      final croppedFace = await _extractionEngine.extractFaceFromFile(filePath: tempFilePath);
      if (croppedFace == null) return false;
      
      final livenessScore = await _passiveLivenessEngine.checkLiveness(croppedFace);
  ...
  Future<EnrollResult> enrollFaceImage({required String imageBase64, bool saveReference = false}) async {
    final tempFilePath = await _writeBase64ToTempFile(imageBase64);
    ...
    try {
      ...
      if (saveReference) {
        await _storage.write(key: "saved_reference_image", value: base64Encode(croppedFace));
        await _storage.write(key: "saved_reference_vector", value: jsonEncode(faceVector));
      }
  ...
  Future<FaceAuthResult> checkFaceAuth({
    ...
    bool? activeLivenessResult; // Passed internally if integrated into UI streams
    ...
    if (useReference) {
      ...
      if (savedImage != null && savedVectorStr != null) {
        ...
        return FaceAuthResult(success: false, strong: false); // No securely saved reference available
      }
    } else if (referenceImageBase64 != null) {
      final enrollResult = await enrollFaceImage(imageBase64: referenceImageBase64, saveReference: false);
      refCropped = enrollResult.croppedFace;
      refVector = enrollResult.faceVector;
    }

    if (refCropped == null || refVector == null) {
      return FaceAuthResult(success: false, strong: false); // Failed to generate reference
    }

    final tempFilePath = await _writeBase64ToTempFile(targetImageBase64);
    try {
      final targetCropped = await _extractionEngine.extractFaceFromFile(filePath: tempFilePath);
      ...
      if (passiveLiveness) {
        ...
        if (!passiveLivenessResult && !proceedIfLivenessFail) {
           return FaceAuthResult(success: false, passiveLivenessResult: passiveLivenessResult); // Spoof detected
        }
      }
      
      // Step 3: ArcFace Local Matching
      final targetVector = await _faceMatchEngine.vectorizeFace(targetCropped);
      final similarity = await _faceMatchEngine.compareVectors(refVector, targetVector);
      ...
      final fallbackResult = await _serverFallbackCompare(refCropped, targetCropped);
      return FaceAuthResult(
  ...
  Future<FaceAuthResult> _serverFallbackCompare(Uint8List refCropped, Uint8List targetCropped) async {
    try {
      final request = http.MultipartRequest("POST", Uri.parse("\$_serverBaseUrl/compare_faces"));
      request.fields['api_key'] = _apiKey ?? "";
      request.fields['cropped'] = "true";
      
      // The FastAPI endpoint is prepared for this format
      request.files.add(http.MultipartFile.fromBytes('reference_image', refCropped, filename: 'ref.jpg'));
      request.files.add(http.MultipartFile.fromBytes('target_image', targetCropped, filename: 'tgt.jpg'));
