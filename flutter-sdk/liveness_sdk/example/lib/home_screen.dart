...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _handleStaticImageEnrollment() async {
    ...
    final result = await widget.sdk.enrollFaceImage(imageBase64: base64Image, saveReference: true);
    if (result.croppedFace != null) {
      _checkEnrollment();
      _showResultDialog("Enrollment Successful", "Face from static image securely stored.");
    } else {
      _showResultDialog("Enrollment Failed", "Could not detect a clear face in the provided image.", success: false);
    }
  }
        ...
        child: Column(
          ...
          children: [
             const Padding(
               ...
               child: Text("Enrollment Options", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
             ),
             ListTile(
               ...
               onTap: () async {
                  ...
                  final result = await Navigator.push(
                    context, 
                    MaterialPageRoute(builder: (_) => EnrollFaceScreen(sdk: widget.sdk, requireActiveLiveness: true))
                  );
                  if (result != null && result is EnrollResult && result.croppedFace != null) {
                    _checkEnrollment();
                    _showResultDialog("Enrollment Successful", "Live face successfully securely enrolled.");
                  }
               },
             ),
             ListTile(
          ...
          return Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text("Authentication Configuration", 
                  ...
                  SwitchListTile(
                    ...
                    onChanged: (val) => setStateModal(() => _requirePassiveLiveness = val),
                  ),
                  
                  SwitchListTile(
                    title: const Text("Require Active Liveness"),
                    value: _requireActiveLiveness,
                    onChanged: (val) => setStateModal(() => _requireActiveLiveness = val),
                  ),
                      ...
                      child: Column(
                        children: [
                          CheckboxListTile(
                            ...
                            value: _activeBlink,
                            onChanged: (val) => setStateModal(() => _activeBlink = val ?? false),
                          ),
                          CheckboxListTile(
                            title: const Text("Nod"),
                            value: _activeNod,
                            onChanged: (val) => setStateModal(() => _activeNod = val ?? false),
                          ),
                          CheckboxListTile(
                            title: const Text("Shake"),
                            value: _activeShake,
                            onChanged: (val) => setStateModal(() => _activeShake = val ?? false),
                          ),
                        ],
                      ),
                    ),
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text("Proceeds to face auth even if spoof detected"),
                    value: _proceedIfLivenessFail,
                    onChanged: (val) => setStateModal(() => _proceedIfLivenessFail = val),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text("Start Authentication", style: TextStyle(fontSize: 18, color: Colors.white)),
                    ),
                  ),
  ...
  Future<void> _launchAuthenticate() async {
     ...
     final result = await Navigator.push(
        context, 
        MaterialPageRoute(builder: (_) => AuthenticateFaceScreen(sdk: widget.sdk))
     );
     
     if (result != null && result is FaceAuthResult) {
        String log = "Auth Match Successful: \${result.success}\\nStrong Model Processed: \${result.strong}";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Biometric SDK Testbed", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
          ...
          children: [
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      color: _isEnrolled ? Colors.green.shade700 : Colors.black54,
                    ),
                  )
                ],
              ),
            ...
            if (!_isEnrolled)
              ElevatedButton(
                ...
                child: const Text("Enroll Face", style: TextStyle(fontSize: 18, color: Colors.white)),
              ),
            ...
            if (_isEnrolled) ...[
              ElevatedButton(
                ...
                child: const Text("Authenticate Face", style: TextStyle(fontSize: 18, color: Colors.white)),
              ),
              const SizedBox(height: 16),
              OutlinedButton(
                ...
                child: const Text("Unenroll", style: TextStyle(fontSize: 18, color: Colors.redAccent)),
              ),
            ]
          ],
        ),
